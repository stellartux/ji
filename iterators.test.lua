...
assert(Table.equallist(iter(), { 32, 33, 34 }))
assert(Table.equallist(iter(), { 34, 35, 36 }))
assert(Table.equallist(iter(), { 36, 37, 38 }))
assert(Table.equallist(iter(), { 38 }))
assert(not iter())

iter = partition(5, 2, take(7, countfrom(39)))
assert(Table.equallist(iter(), { 39, 40, 41, 42, 43 }))
assert(Table.equallist(iter(), { 41, 42, 43, 44, 45 }))
assert(Table.equallist(iter(), { 43, 44, 45 }))
assert(Table.equallist(iter(), { 45 }))
assert(not iter())
